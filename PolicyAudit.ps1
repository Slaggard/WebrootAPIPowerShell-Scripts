...
##  v4.2 - Added User-Agent header to web requests per Joseph Ri at Webroot.
...
# Version number of this script, should be changed whenever script is modified
$ScriptVersion = "4.2";
# User agent string for web requests, never changes
$UserAgent = "ConceptPowershellPolicyAudit/$ScriptVersion";
...
Function Get-Sites {
	...
	Do {
		$i++; #increment counter
		$Request = Invoke-WebRequest -Uri $SitesUri -UserAgent $UserAgent -Header $header -ContentType 'application/json' -Method Get -TimeoutSec 30;	
		#$Request.StatusCode;
		...
		Do { #Get 50 at a time
			...
			Do { #Retry page request until success
				...
				Try {
					...
					$Request = Invoke-WebRequest -Uri $EndpointIDURL -UserAgent $UserAgent -Header $header -ContentType 'application/json' -Method Get -TimeoutSec 20;	
				} catch {
			...
			$MyOutput = $Endpoints |
				...
				Select-Object -Property SiteName,HostName,PolicyName,PolicyId,WindowsFullOS,Mac,MACAddress,LastSeen,AgentVersion,EndpointId,SiteId | 
